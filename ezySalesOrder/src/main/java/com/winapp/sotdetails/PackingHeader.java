...
public class PackingHeader extends SherlockFragmentActivity implements SlideMenuFragment.MenuClickInterFace {
	...
	private LinearLayout spinnerLayout, packed_parent, searchlayout, codelayout;
	...
	private String companyCode="", valid_url="", serverdate, sDate, eDate, cuscode="", select_van="", mSoNo="", mIsQtyVerified="", status="";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.packing_header);
		...
		custsearchIcon.setImageResource(R.mipmap.refresh);
		...
		packed_parent = (LinearLayout) findViewById(R.id.packed_parent);
		codelayout = (LinearLayout) findViewById(R.id.codelayout);
		searchlayout = (LinearLayout) findViewById(R.id.searchlayout);
		spinner_status = (Spinner) findViewById(R.id.status_spn);
		...
		helper = new UIHelper(PackingHeader.this);
		FWMSSettingsDatabase.init(PackingHeader.this);
		valid_url = FWMSSettingsDatabase.getUrl();
		new DateWebservice(valid_url,PackingHeader.this);
		new SalesOrderWebService(valid_url);
		sotdb = new SOTDatabase(PackingHeader.this);
		SOTDatabase.init(PackingHeader.this);
		
		// @Offline
				onlineMode = OfflineDatabase.getOnlineMode();
				offlineDatabase = new OfflineDatabase(PackingHeader.this);
				offlineCommon = new OfflineCommon(PackingHeader.this);
				checkOffline = OfflineCommon.isConnected(PackingHeader.this);
				OfflineDatabase.init(PackingHeader.this);
				offlineLayout = (LinearLayout) findViewById(R.id.inv_offlineLayout);
				spManager = new OfflineSettingsManager(PackingHeader.this);
...
//				}
		...
		starteditTextDate.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (MotionEvent.ACTION_UP == event.getAction())
					new DatePickerDialog(PackingHeader.this, startDate,
		...
		endeditTextDate.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (MotionEvent.ACTION_UP == event.getAction())
					new DatePickerDialog(PackingHeader.this, endDate,
		...
		btcstmrsrch.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				spinnerstatus();
				if (sDate.matches("")) {
					Toast.makeText(PackingHeader.this, "Enter Start Date",
							Toast.LENGTH_SHORT).show();
				} else if (eDate.matches("")) {
					Toast.makeText(PackingHeader.this, "Enter End Date",
							Toast.LENGTH_SHORT).show();
				} else {
		...
		searchIcon.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				InputMethodManager inputMethodManager = (InputMethodManager) PackingHeader.this
	...
	public void spinnerstatus() {
		String statusStr = spinner_status.getSelectedItem().toString();
		Log.d("statusStr", "-->" + statusStr);
		if (statusStr.matches("Open")) {
			status = "0";
		} else if (statusStr.matches("In Progress")) {
			status = "1";
		} else if (statusStr.matches("Packed")) {
			status = "2";
		} else if (statusStr.matches("All")) {
		 status = "3";
		 }
	...
	public void alertDialogSearch() {
		AlertDialog.Builder myDialog = new AlertDialog.Builder(
				PackingHeader.this);
		final EditText editText = new EditText(PackingHeader.this);
		final ListView listview = new ListView(PackingHeader.this);
		LinearLayout layout = new LinearLayout(PackingHeader.this);
		...
		arrayAdapterSupp = new CustomAlertAdapterSupp(PackingHeader.this, al);
		...
		editText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				arrayAdapterSupp = new CustomAlertAdapterSupp(
						PackingHeader.this, searchResults);
				listview.setAdapter(arrayAdapterSupp);
			}
		});
...
//			}	
		...
		protected void onPostExecute(Void result) {
			...
			if (serverdate != null) {
				...
				enableViews(packed_parent, true);
			}
...
//				} else {  //Online
					...
					list = SalesOrderWebService.getPackedHeaderList(params, "fncGetInvoiceHeader", status);
//				}
//			} else if (onlineMode.matches("False")) {  // permanent_offline
//				
//				  try {
...
//			}	
		...
		protected void onPostExecute(Void result) {
			...
			if (mHeaderArr.size()>0 && mDetailsArr.size()>0) {
				Intent i = new Intent(PackingHeader.this, PackingDetails.class);
				i.putExtra("SOHeader", mHeaderArr);
				i.putExtra("SODetails", mDetailsArr);
				startActivity(i);
				PackingHeader.this.finish();
			}else{
				
				Toast.makeText(PackingHeader.this, " No data found-> ", Toast.LENGTH_LONG).show();
			}
				progressBar.setVisibility(View.GONE);
				spinnerLayout.setVisibility(View.GONE);
				enableViews(packed_parent, true);
	...
	public void searchCustCode() throws ParseException {
		...
		soadapter = new HeaderAdapter(PackingHeader.this,R.layout.packing_header_list_item, null, list);
...
//	}
	...
	public void loadprogress(){
		spinnerLayout = new LinearLayout(PackingHeader.this);
		spinnerLayout.setGravity(Gravity.CENTER);
		PackingHeader.this.addContentView(spinnerLayout,
				new LayoutParams(
						ViewGroup.LayoutParams.FILL_PARENT,
						ViewGroup.LayoutParams.FILL_PARENT));
		spinnerLayout.setBackgroundColor(Color.parseColor("#80000000"));
		enableViews(packed_parent, false);
		progressBar = new ProgressBar(PackingHeader.this);
	...
	public void onBackPressed() {
		Intent i = new Intent(PackingHeader.this, LandingActivity.class);
		startActivity(i);
		PackingHeader.this.finish();
	}
